    ...
    for (int iter = 0; iter < max_iterations; ++iter)
    {
        ...
        else
        {
            ...
            if (visualizer)
            {
                ...
                visualizer->publishClusteredFrontiers(frontier_clusters, octomap_resolution, false, 0.85f, "frontier_clusters");
        ...
        for (size_t i = 0; i < reachable_viewpoints.size(); ++i)
        {
            double info_gain = computeInformationGain(
                ...
                octomap_interface,
                geometry_utils::rad2Deg(camera_horizontal_fov), // Convert radians to degrees
                camera_width,
                camera_height,
                camera_max_range,
                octomap_resolution,
        ...
        // Get current joint state once to use as IK seed
        ...
        std::optional<std::vector<double>> best_joint_angles;
        ...
        while (!viewpoint_heap.empty() && !found_valid_viewpoint)
        {
            ...
            std::vector<double> ik_solution = interface->computeIK(
                current_joint_state,
                target_ee_pose,
                5.0, // timeout
                10   // attempts
            ...
            if (!ik_solution.empty())
            {
                ...
                {
                    best_joint_angles = ik_solution;
                    found_valid_viewpoint = true;
                    RCLCPP_INFO(node->get_logger(), "Found valid viewpoint with collision-free IK solution!");
                }
                else
                {
                    RCLCPP_DEBUG(node->get_logger(), "  IK solution is in collision, trying next viewpoint");
                }
            }
            else
            {
                RCLCPP_DEBUG(node->get_logger(), "  No IK solution found, trying next viewpoint");
            }
        }
        if (!found_valid_viewpoint)
        {
            RCLCPP_WARN(node->get_logger(), "No valid viewpoint with IK solution found!");
        }
        else
        {
